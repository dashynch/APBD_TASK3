...
public sealed class LinqExercises
{
    ...
    {
        return UniversityData.Students.Where(s => s.City == "Warsaw").Select(
            e => $"{e.IndexNumber} {e.FirstName} {e.LastName} {e.City}").ToList();
    }
    ...
    {
        return [UniversityData.Courses.Where(c => c.Category == "Analytics").Select(e => 
            $"{e.Title} {e.StartDate}").FirstOrDefault() ?? "No course from the Analytics found."];
    }
    ...
    {
        return UniversityData.Students.Select(c => c.City).Distinct().OrderBy(e => e).ToList();
    }
    ...
    {
        return UniversityData.Enrollments.OrderByDescending(c => c.EnrollmentDate).Take(3)
            .Select(c => $"{c.EnrollmentDate:yy-MM-dd} {c.StudentId} {c.CourseId}").ToList();
    }
    ...
    {
        return UniversityData.Students.Join(UniversityData.Enrollments, c => c.Id, e => e.StudentId,
            (c, e) => $"{c.FirstName} {c.LastName} {e.EnrollmentDate:yy-MM-dd}").ToList();
    }
    ...
    public IEnumerable<string> Task12_StudentCoursePairs()
    {
        return UniversityData.Enrollments
            .Join(UniversityData.Students, e => e.StudentId, c => c.Id, (e, c) => new { e, c })
            .Join(UniversityData.Courses, a => a.e.CourseId, b => b.Id,
                (a, b) => $"{a.c.FirstName} {a.c.LastName} {b.Title}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Join(UniversityData.Courses, e => e.CourseId, c => c.Id, (e, c) => c.Title)
            .GroupBy(title => title).Select(a => $"{a.Key} {a.Count()}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Where(c => c.FinalGrade != null)
            .Join(UniversityData.Courses, c => c.CourseId, e => e.Id, (c, e) => new { e.Title, c.FinalGrade })
            .GroupBy(a => a.Title).Select(b => $"{b.Key} {b.Average(a => a.FinalGrade):F2}").ToList();
    }
    ...
    {
        return UniversityData.Lecturers.GroupJoin(UniversityData.Courses, c => c.Id, e => e.LecturerId,
            (c, e) => $"{c.FirstName} {c.LastName} {e.Count()}").ToList();
    }
    ...
    public IEnumerable<string> Task16_HighestGradePerStudent()
    {
        return UniversityData.Students
            .Join(UniversityData.Enrollments, c => c.Id, e => e.StudentId, (c, e) => new { c, e })
            .Where(a => a.e.FinalGrade != null).GroupBy(a => new { a.c.FirstName, a.c.LastName })
            .Select(b => $"{b.Key.FirstName} {b.Key.LastName} {b.Max(a => a.e.FinalGrade)}").ToList();
    }
    ...
    {
        return UniversityData.Students.Join(UniversityData.Enrollments.Where(c => c.IsActive), e => e.Id, 
                a => a.StudentId, (e, a) => new { e.FirstName, e.LastName }).GroupBy(a => new { a.FirstName, a.LastName })
            .Where(b => b.Count() > 1).Select(b => $"{b.Key.FirstName} {b.Key.LastName} {b.Count()}").ToList();
    }
    ...
    public IEnumerable<string> Challenge02_AprilCoursesWithoutFinalGrades()
    {
        return UniversityData.Courses.Where(c => c.StartDate.Month == 4 && c.StartDate.Year == 2026)
            .GroupJoin(UniversityData.Enrollments, c => c.Id, e => e.CourseId,
                (c, e) => new { c.Title, HasGrades = e.Any(e => e.FinalGrade != null) }).Where(a => !a.HasGrades)
    ...
    public IEnumerable<string> Challenge03_LecturersAndAverageGradeAcrossTheirCourses()
    {
        return UniversityData.Lecturers
            .Join(UniversityData.Courses, c => c.Id, e => e.LecturerId, (c, e) => new { c, e })
            .Join(UniversityData.Enrollments.Where(l => l.FinalGrade != null), a => a.e.Id, l => l.CourseId,
                (a, l) => new { a.c.FirstName, a.c.LastName, l.FinalGrade })
            .GroupBy(a => new { a.FirstName, a.LastName })
            .Select(b => $"{b.Key.FirstName} {b.Key.LastName} {b.Average(a => a.FinalGrade):F2}").ToList();
    }
    ...
    {
        return UniversityData.Students
            .Join(UniversityData.Enrollments.Where(c => c.IsActive), e => e.Id, c => c.StudentId, (e, c) => e.City)
            .GroupBy(city => city).OrderByDescending(b => b.Count()).Select(b => $"{b.Key} {b.Count()}").ToList();
    }
